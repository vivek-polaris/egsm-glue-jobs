...
def process_power_events(event_df):
    ...
    return df.filter(
        (col("event_code") == EVENT_ON) & (col("prev_event_code") == EVENT_OFF) & col("prev_event_time").isNotNull()) \
...
def handle_exceptions(event_df, block_df):
    ...
    df = spark.createDataFrame(result_rows,
                               ["MeterId", "Power_Off_Datetime", "First_power_ON_after_Power_Off_datetime"])
    df = df.withColumn("duration_seconds",
                       unix_timestamp("First_power_ON_after_Power_Off_datetime") - unix_timestamp("Power_Off_Datetime"))
...
def format_output(df):
    return df.withColumn(
        ...
        .select("MeterId", "Power_Off_Datetime", "First_power_ON_after_Power_Off_datetime", "Duration (Days, HH:MM:SS)",
                "year", "month")
